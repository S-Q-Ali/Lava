...
## D-033 — Proxy previews as a dedicated backend service (M9 proxy-preview)

- **Date**: 2026-09-14
- **WHAT**: Module 1 of M9. Image/video proxies are generated by the backend at
  `POST/GET /api/proxy` and consumed lazily by the frontend; preview uses the
  proxy while the render path continues to use original assets.
- **WHY**: The baseline HP Pavilion (hexa-core i7 10th gen, MX250 2 GB) cannot
  decode or animate full-resolution media at preview time without dropped
  frames or OOM on tall manhwa strips. A deterministic, cacheable downscale
  keeps the preview pipeline cheap without touching render semantics.
- **HOW**: proxyId is the `sha256(data).hexdigest()[:16]`; identical inputs are
  served from the same cache file (idempotent). Image proxies are WebP
  (quality 80, max width 480, max height 960, no upscale); video proxies are
  MP4 (`scale=-2:min(ih,480)` with escaped comma, 15 fps, max 120 s, libx264
  veryfast crf 28, `-an`, `+faststart`). Proxies live under `Config.proxy_dir`
  (`cache/backend/proxy`). The frontend `resolveProxyUrl` keeps a module cache
  keyed by blob URL, fires one request per asset (pending-set dedupe) and
  bumps a transient `useProxyStore` version so the PreviewPanel swaps to the
  proxy when it lands; offline/no-sidecar always falls back to the original
  URL. `Asset.proxyUrl` is optional metadata; render never consults it.
- **Alternatives considered**: eager proxy generation at import time (rejected —
  slows import and burns I/O on files never previewed); client-side canvas
  downscale (rejected — duplicates the decode cost on the same weak machine);
  OneDrive/network cache (rejected — violates local-first).
