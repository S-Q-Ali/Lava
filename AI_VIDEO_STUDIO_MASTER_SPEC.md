...
Big Pickle + OpenCode Build Specification
...
GRAPHIFY
Purpose: map and query the evolving codebase so the coding agent can reason about architecture and dependencies.
Expected project integration: project-scoped Graphify installation and generated graph artifacts.

ADDY OSmani AGENT SKILLS
Purpose: disciplined engineering workflows such as specification, planning/task breakdown, incremental implementation, testing and code review.

ANTHROPIC SKILLS
Purpose: specialized workflow patterns. Use selectively where relevant instead of loading every skill for every task.

BIG PICKLE
Purpose: primary implementation agent. It must use the skill system and repository knowledge rather than relying on one giant prompt.

OPEN CODE
Purpose: orchestrate the agent, skills, tools, repository operations and verification.

## 4. User Decisions — Locked Product Behavior

Voice-over/image sync:
- Full automatic mode is required.
- Automatic matching must also provide confidence information and remain manually editable.
...
- Default behavior should favor clean cuts and restrained transitions; effects must not be inserted merely because they exist.
...
The system should not mechanically enforce one sentence = one image.

## 6. Retention-Oriented Editing Engine

The engine must NOT claim that a transition makes a video viral. Instead it should optimize measurable editing heuristics.
...
Goal:
Drop one very tall Manhwa/Webtoon image and automatically detect its visual panels, preserving original resolution.

The supplied examples establish the intended class of input: long vertical-scroll webtoon/manhwa layouts where panels can vary greatly in height, gutters can create pacing, and panels may be borderless or visually connected.
...
The user was unsure about the phrase “combined panels”, so implement it visually rather than exposing confusing terminology.
...
Create a panel asset collection:
panel_001.png
panel_002.png
panel_003.png
...
...
- confidence
- order
- optional user-corrected flag

Also support JPG export, with PNG as the lossless/default option.

## 10. Captions System

All major caption modes requested by the user are in scope:
- Normal subtitles
...
Caption generation:
ASR → timestamps → sentence/phrase segmentation → word timestamps where supported → style renderer → editable caption track.

Every caption should remain editable after AI generation.
...
Trending, New, Shorts, Reels, YouTube, Anime, Manhwa, Storytelling, Cinematic, Motivation, Meme, Documentary, Custom.
...
The editor should provide a professional timeline rather than a card dashboard.
...
If additional tracks are needed by implementation, add them only when they provide real workflow value.
...
Default behavior:
AI does the first pass quickly.
User can inspect and override.
The editor preserves user changes.
...
Local-first directories should be configurable:
.venv/
node_modules/
models/
cache/
...
## 18. Big Pickle Operating Procedure
...
- Keep architecture documented.

PHASE 4 — VERIFY
- Run unit tests.
- Run integration tests.
- Run lint/type checks where applicable.
- Run feature-specific fixtures.
- Check performance on baseline hardware where relevant.
...
- Check that AI operations are reversible.
- Update Graphify/context/documentation.
...
- UI path exists where applicable
- error states exist
- manual override exists where required
- tests exist
- tests pass
- performance is acceptable for the target hardware or limitation is documented
- documentation is updated
- no unrelated regressions are introduced
- Graphify/project knowledge is updated when architecture changed
...
- project-local environments/cache strategy
