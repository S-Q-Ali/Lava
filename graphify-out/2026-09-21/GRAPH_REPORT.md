# Graph Report - Lava  (2026-09-21)

## Corpus Check
- 366 files · ~346,311 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4503 nodes · 7819 edges · 283 communities (259 shown, 15 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 283 edges (avg confidence: 0.93)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `03e58aed`
...
1. `ApiError` - 87 edges
2. `useEditorStore` - 85 edges
3. `react` - 61 edges
4. `get_config()` - 59 edges
5. `AI Video Studio — Session Log` - 53 edges
6. `vitest` - 47 edges
7. `backendBaseUrl()` - 41 edges
8. `post()` - 40 edges
9. `ManhwaError` - 37 edges
10. `detect_strip()` - 36 edges
...
## Communities (283 total, 15 thin omitted)

### Community 0 - "package.json"
Cohesion: 0.05
Nodes (39): dependencies, react, react-dom, zundo, zustand, devDependencies, jsdom, oxlint (+31 more)

### Community 1 - "transitions.ts"
Cohesion: 0.08
Nodes (29): betweenLabel(), clipName(), imageA, imageB, TransitionsPanel(), BetweenTransition, clampTransitionDuration(), ClipLike (+21 more)
...
Cohesion: 0.10
Nodes (27): ExportPanel(), FPS_OPTIONS, FpsOption, PLATFORM_PRESETS, PlatformPreset, DISPLAY_MODES, DisplayMode, formatTime() (+19 more)
...
### Community 59 - "Transcription"
Cohesion: 0.17
Nodes (8): FakeTranscriber, Path, Segment, Transcriber, Transcription, WhisperTranscriber, make_client(), TestTranscribeEndpoint
...
Cohesion: 0.09
Nodes (16): downloadAs(), ExtractedImage, ExtractedPage, ExtractorPanel(), getExt(), loadImage(), ImageToImagePanel(), WorkspaceOverview() (+8 more)
...
### Community 65 - ".load"
Cohesion: 0.13
Nodes (12): asset_for_id(), asset_name(), id_for_asset(), Path, Zero-padded asset file name, e.g. `panel_001.png`. Width = len(str(total))., Default id ↔ asset mapping: `panel_003.png` → `p3`. Ids are stable; asset file…, parametrize, Path (+4 more)
...
Cohesion: 0.09
Nodes (29): crop_panel(), encode_panel(), _export_name(), ExportBundle, ExportFile, iter_export_files(), _iter_files(), manifest_rows() (+21 more)
...
### Community 94 - "InspectorPanel.tsx"
Cohesion: 0.14
Nodes (10): AIMatchPanel(), AIMatchTab, InspectorPanel(), RightPanel(), attachPauses(), buildNodes(), confidenceLabel(), lowConfidence() (+2 more)
...
Cohesion: 0.10
Nodes (46): error_response(), is_allowed_font_name(), make_font_metadata(), _all_presets(), api_error_handler(), _builtin_by_id(), ClipMetadata, create_preset() (+38 more)
...
Cohesion: 0.16
Nodes (22): FLAGS, TemplateEditorPanel(), overwrite(), patch(), saveAsNew(), selectBase(), ANIMATION_OPTIONS, CAPTION_STYLES (+14 more)
...
Cohesion: 0.08
Nodes (17): BlendEntry, CloneStatus, VoiceBlendingPanel(), DEFAULT_BASE_URL, detectProvider(), FFmpegProvider, FontRenderInfo, HttpFFmpegProvider (+9 more)
...
Cohesion: 0.07
Nodes (26): _apply_panels(), add_panel(), adjust_panel(), delete_panel(), merge_panels(), Replace one panel's bounds (crop/expand within the source). The new box must…, Insert a user-defined panel (near-certain: confidence 1.0). The new panel gets…, Apply an explicit reading sequence: ids must be an exact permutation. Every… (+18 more)
...
Cohesion: 0.06
Nodes (81): Config, _find_repo_root(), _payload(), Path, Resolve Lava Studio project-local configuration and paths., Walk up from *start* to find the repo root (has .git directory)., Resolve a binary path, trying .exe suffix on Windows., reset_config() (+73 more)
...
Cohesion: 0.12
Nodes (23): MatchPanel(), hasTimingOverride(), MIN_AUTO_DURATION, pacedEnd(), TAIL_HOLD, TimedBeat, TIMING_EPSILON, getAssetFile() (+15 more)

### Community 127 - "load_analysis_image"
Cohesion: 0.11
Nodes (22): _bg_estimate(), detect_cuts(), load_analysis_image(), ndarray, Background color = mode of the 1px outer ring (comic margins). Panels carry…, 1..SEAM_MAX_H flat empty runs between clean bands, bordered by content., Convert to gray float analysis image; returns (gray, ana_w, ana_h, factor).…, _rescue_cuts() (+14 more)

### Community 128 - "AI Video Studio — Session Log"
Cohesion: 0.13
Nodes (15): AI Video Studio — Session Log, Decisions, HOW, HOW, Limitations, Limitations, Next step, Next step (+7 more)

### Community 129 - "detect_strip"
Cohesion: 0.14
Nodes (16): detect_strip(), Any, Detect panels in a vertical strip; optionally persist crops + registry.…, Memory-tuning tests: JPEG draft decode and lazy open path., In-memory Image (no fp) falls back to full convert, never attempts draft., Create a wide tall JPEG strip that must downscale for the analysis path. Width…, Draft decode must not change the panel layout vs a full decode., _tall_jpeg() (+8 more)
...
Cohesion: 0.17
Nodes (18): CaptionStyle, BUILTIN_CATEGORIES, isCategory(), parsePreset(), Preset, PresetCategory, good, deletePreset() (+10 more)
...
Cohesion: 0.17
Nodes (11): _get_transcriber(), Return the app transcriber, building it lazily on first use. A pre-injected…, fixture, _reset_model_state(), test_config_reads_render_timeout_seconds(), test_health_responds_during_render(), test_lazy_factories_respect_injected_fakes(), test_lazy_matcher_built_once_and_reused() (+3 more)
...
Cohesion: 0.09
Nodes (42): get_config(), Path, Cross-platform file system helpers., Remove a directory tree, with Windows-friendly retry logic. On Windows, files…, rmtree_safe(), apply_correction(), delete_strip(), detect_panels() (+34 more)
...
Cohesion: 0.09
Nodes (34): ASPECT_RATIOS, AspectRatio, mount(), mountWithProps(), TopBar(), TopBarProps, isAsset(), isClip() (+26 more)
...
Cohesion: 0.12
Nodes (15): _ml_detections_to_panels(), Convert ML bounding box detections to Panel objects. Filters to panel-class…, attribute_confidence(), _certain_confidence(), guard_layout(), order_panels(), M7 module 3: reading order, confidence attribution, layout guards. Pure data…, Sort into natural top→bottom reading order and renumber `order` 1..n. Sort key… (+7 more)
...
### Community 157 - "post"
Cohesion: 0.23
Nodes (15): BulkTTSRequest, BulkTTSResponse, BulkTTSResult, download_zip(), generate_bulk(), _generate_one(), BaseModel, UploadFile (+7 more)
...
Cohesion: 0.16
Nodes (8): _png_bytes(), M7 module 6: manhwa-api HTTP tests (isolated tmp storage)., TestDeleteStrip, TestExport, TestPanelServe, TestReadEndpoints, TestUploadErrors, _upload()
...
### Community 174 - "editorStore.ts"
Cohesion: 0.13
Nodes (25): CAPTION_PAUSE_SPLIT_THRESHOLD, CAPTION_TRACK_ID, CaptionItem, CaptionSource, CaptionWord, clampCaptionDuration(), DEFAULT_CAPTION_STYLE_ID, isCaption() (+17 more)
...
Cohesion: 0.19
Nodes (8): _parse_groq_response(), Parse Groq Whisper API response into our Transcription model., make_client(), _make_groq_response(), patch, Build a mock Groq API response dict., TestGroqTranscribeEndpoint, TestParseGroqResponse
...
Cohesion: 0.18
Nodes (11): 1. Repository Layout, 2. Design Principles, 3. Core Technology, 4. Voice-over → Images Processing Graph, 5. Manhwa / Webtoon Extraction Processing Graph, 6. Caption Processing Graph, 7. M11/M12 Backend Modules, 7. System Boundaries (+3 more)
...
Cohesion: 0.14
Nodes (18): StyleOption, STYLES, VisualPromptPanel(), CaptionExportFormat, exportCaptions(), generateVisualPrompts(), GroqTranscribeOptions, parseTranscript() (+10 more)
...
### Community 189 - "ops.ts"
Cohesion: 0.11
Nodes (23): addClip(), addClips(), clipsAtTime(), createClip(), duplicateClip(), moveClip(), newId(), removeClip() (+15 more)
...
### Community 194 - "NavRail.test.tsx"
Cohesion: 0.33
...
### Community 198 - "ApiError"
Cohesion: 0.08
Nodes (30): check_for_updates(), BaseModel, get, Auto-Update — check GitHub releases for new versions. Compares current version…, Compare semver strings. Returns >0 if v1 > v2, <0 if v1 < v2, 0 if equal., Check GitHub releases for a new version. Returns current vs latest version,…, UpdateInfo, _version_compare() (+22 more)
...
### Community 207 - "ManhwaDetector"
Cohesion: 0.15
Nodes (14): DetectionResult, ManhwaDetector, PanelDetection, Path, ML-based manhwa panel detection using YOLO models. Tiered approach: - Tier 1:…, Load the segmentation model. Returns True if successful., Detect panels using nano model. Returns None if unavailable., Detect panels using segmentation model. Returns None if unavailable. (+6 more)
...
Cohesion: 0.15
Nodes (30): correctStrip(), deleteStrip(), detectStrip(), getStrip(), listStrips(), ManhwaError, manhwaFetch(), ManhwaPanel (+22 more)
...
### Community 216 - "timeline_sync.py"
Cohesion: 0.27
Nodes (10): _get_audio_duration(), _parse_timecode_from_filename(), BaseModel, Timeline Auto-Sync — sync timecoded images to voiceover audio. Detects…, Extract timecode from filename like '0-00.png', '0-03.png', 'scene_1_45s.png'., Get audio duration in ms using ffprobe., Sync images to voiceover audio timeline. - audio_file: Path to voiceover audio…, sync_timeline() (+2 more)
...
### Community 220 - "types.ts"
Cohesion: 0.08
Nodes (29): MOTION_TYPES, MotionPanel(), applyClips(), clipWith(), imageAsset, videoAsset, Beat, labelFor() (+21 more)
...
Cohesion: 0.08
Nodes (23): delete_model(), _download_model(), download_progress(), _DownloadProgressTqdm, _get_progress(), model_status(), ModelInfo, ModelStatusResponse (+15 more)

### Community 224 - "voice_cloning.py"
Cohesion: 0.17
Nodes (17): clone_status(), clone_voice(), _clone_voice_simple(), CloneResult, CloneStatusResponse, download_clone(), _get_model_path(), _is_model_available() (+9 more)

### Community 225 - "Panel"
Cohesion: 0.24
Nodes (14): _fresh_id(), M7 module 5: pure panel correction ops (module 5 slice 2). Split / Merge /…, Re-run detection (module 2) and return the fresh, guarded panel list.…, redetect(), _require(), _expect_float(), _expect_int(), _expect_str() (+6 more)

### Community 226 - "detect_pauses"
Cohesion: 0.35
Nodes (5): detect_pauses(), Pause, Word, TestDetectPauses, words()

### Community 227 - "Modules"
Cohesion: 0.17
Nodes (11): Boundaries, Module 1: Script Templates (deferred-features), Module 2: My Generations (deferred-features), Module 3: Auto-Update (deferred-features), Module 4: Pipeline Polish (pipeline-polish), Module 5: Documentation (spec-docs), Modules, Objective (+3 more)
...
### Community 231 - "react"
Cohesion: 0.06
Nodes (26): App(), AssetCategory, AssetGrid(), sortRecent(), AssetsPanel(), BottomTab, tabDefs, tabToCategory() (+18 more)
...
### Community 236 - "Implementation Plan: M12 Remaining Work"
Cohesion: 0.18
Nodes (10): Build Order (dependency graph), Checkpoints, Implementation Plan: M12 Remaining Work, Overview, Phase 1: Quality Bar (1 task), Phase 2: Deferred Features (3 tasks, parallel), Phase 3: Pipeline Polish (2 tasks), Phase 4: Documentation (2 tasks) (+2 more)

### Community 237 - "make_panel"
Cohesion: 0.36
Nodes (3): make_panel(), _panel(), TestPanel

### Community 239 - "test_render_license.py"
Cohesion: 0.28
...
Cohesion: 0.08
Nodes (28): BulkGenerationPanel(), BulkResult, ClipperPanel(), ClipResult, GeneratedImage, ImageGenPanel(), LeftWorkspaceProps, Generation (+20 more)
...
### Community 252 - "detect.py"
Cohesion: 0.20
Nodes (13): _analysis_array(), _bordered_by_content(), find_gutter_bands(), GutterBand, _open_source(), Image, Path, M7 module 2: hybrid panel detection (OpenCV + numpy). Operates on a downscaled… (+5 more)

### Community 253 - "build_panels"
Cohesion: 0.29
Nodes (6): build_panels(), Cut, merge_slivers(), Drop lower-confidence cuts that create sub-MIN_PANEL_H interior panels., Convert analysis-space cuts into seam-free source-space incident panels. Cut…, TestBuildPanels
...
### Community 265 - "Module Details"
Cohesion: 0.22
Nodes (8): Capability Map, Capability Map: M12 Remaining Work, Context, Module: deferred-features, Module Details, Module: pipeline-polish, Module: quality-bar, Module: spec-docs

### Community 266 - "12. M11/M12 Features (AutoCut Studio PRO + Clabeo Parity)"
Cohesion: 0.25
Nodes (8): 12. M11/M12 Features (AutoCut Studio PRO + Clabeo Parity), Audio Features, Captions, Image Generation, Pipeline, Tools, Video Features, Voice & TTS

### Community 267 - "SettingsPanel.tsx"
Cohesion: 0.19
Nodes (10): formatMB(), HardwareAnalysis, HardwarePanel(), HardwareSpecs, ModelAssessment, SCENARIO_META, formatSize(), ModelInfo (+2 more)

### Community 268 - "M12 — Remaining Work (spec-first, SPEC-m12-remaining.md)"
Cohesion: 0.40
Nodes (5): M12 — Remaining Work (spec-first, SPEC-m12-remaining.md), Phase 1: Quality Bar, Phase 2: Deferred Features (parallel), Phase 3: Pipeline Polish, Phase 4: Documentation

### Community 269 - "Clip"
Cohesion: 0.13
Nodes (16): DragMode, PX_PER_SECOND, TRACK_HEIGHT, TimelinePanel(), TrackRow(), imageA, imageB, TransitionOverlay() (+8 more)

### Community 270 - "analysis_scale"
Cohesion: 0.39
Nodes (3): analysis_scale(), Downscale factor for the detection analysis image. Returns `(ana_w, ana_h,…, TestAnalysisScale

### Community 272 - "vitest"
Cohesion: 0.07
Nodes (13): AutoCaptionsPanel(), formatTimestamp(), LANGUAGES, CaptionPanel(), transcript, voice, PresetPanel(), builtin (+5 more)

### Community 273 - "groq_transcribe.py"
Cohesion: 0.20
...
### Community 274 - "lava_backend/transcribe.py"
Cohesion: 0.26
Nodes (7): confidence_from_logprob(), Request, UploadFile, serialize(), serialize_segment(), transcribe(), TestConfidenceFromLogprob

### Community 275 - "ManhwaPanel.tsx"
Cohesion: 0.20
Nodes (6): ManhwaPanelRow(), ManhwaPanelRowProps, stripDetail, stripSummary, CorrectionOp, panelImageUrl()

### Community 276 - "purge_stale_proxies"
Cohesion: 0.28
...
### Community 279 - "Session 2026-09-20 — ML-Based Manhwa Panel Detection"
Cohesion: 0.25
Nodes (8): Decisions, HOW, Limitations, Next step, Purpose (WHY), Session 2026-09-20 — ML-Based Manhwa Panel Detection, Verify, WHAT

### Community 280 - "Session 2026-09-21 — Settings modal skeleton loading"
Cohesion: 0.25
Nodes (8): Decisions, HOW, Limitations, Next step, Purpose (WHY), Session 2026-09-21 — Settings modal skeleton loading, Verify, WHAT

### Community 282 - "Session 46 — V2 Frontend Gap Fixes: Phase 7 (Auto Captions Polish)"
Cohesion: 0.33
Nodes (6): Files changed, Next step, Purpose (WHY), Session 46 — V2 Frontend Gap Fixes: Phase 7 (Auto Captions Polish), Verify, WHAT

## Knowledge Gaps
- **1753 isolated node(s):** `$schema`, `plugin`, `lava-backend`, `$schema`, `plugins` (+1748 more)
...
- **Why does `ApiError` connect `ApiError` to `voice_cloning.py`, `main.py`, `image_gen.py`, `podcast_maker.py`, `test_runtime.py`, `groq_transcribe.py`, `lava_backend/transcribe.py`, `video_clipper.py`, `generators.py`, `get_config`, `sound_fx.py`, `media.py`, `voice_library.py`, `post`, `timeline_sync.py`?**
  _High betweenness centrality (0.023) - this node is a cross-community bridge._
- **Why does `ManhwaError` connect `ManhwaError` to `Panel`, `detect_strip`, `.load`, `export.py`, `_panel`, `validate_layout`, `get_config`, `order.py`, `detect.py`?**
  _High betweenness centrality (0.012) - this node is a cross-community bridge._
- **Why does `react` connect `react` to `package.json`, `transitions.ts`, `editor/presets.ts`, `PreviewPanel.tsx`, `SettingsPanel.tsx`, `Clip`, `vitest`, `ManhwaPanel.tsx`, `project.ts`, `PresetPanel.test.tsx`, `voice.ts`, `useEditorStore`, `NavRail.test.tsx`, `types.ts`, `InspectorPanel.tsx`, `VoiceoverPanel.tsx`, `templateEditor.ts`, `ScriptWriterPanel.tsx`, `ffmpeg.ts`, `LeftWorkspace.tsx`, `FontPanel.tsx`, `matchingStore.ts`?**
  _High betweenness centrality (0.011) - this node is a cross-community bridge._
- **Are the 42 inferred relationships involving `ApiError` (e.g. with `download_zip()` and `generate_bulk()`) actually correct?**
  _`ApiError` has 42 INFERRED edges - model-reasoned connections that need verification._
