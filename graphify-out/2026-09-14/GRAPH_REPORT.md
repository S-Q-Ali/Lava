...
## Corpus Check
- 202 files · ~154,833 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2585 nodes · 4372 edges · 162 communities (148 shown, 12 thin omitted)
- Extraction: 97% EXTRACTED · 3% INFERRED · 0% AMBIGUOUS · INFERRED: 131 edges (avg confidence: 0.94)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f2b93787`
...
Cohesion: 0.15
Nodes (22): betweenLabel(), clipName(), BetweenTransition, clampTransitionDuration(), ClipLike, DEFAULT_DURATIONS, defaultDuration(), EvaluateOptions (+14 more)
...
### Community 8 - "ops.ts"
Cohesion: 0.10
Nodes (29): InspectorPanel(), kindOf(), MediaPanel(), TimelinePanel(), addClip(), addClips(), ClipInput, createClip() (+21 more)
...
Cohesion: 0.07
Nodes (30): AI Video Studio — Decision Log, D-001 — Web-first editor shell (Vite + React + TypeScript), D-002 — Zustand + Zundo for state and undo/redo, D-003 — Project-local FFmpeg binary, D-004 — FFmpeg provider abstraction, D-005 — Curated vendor skills; reference clones kept local, D-006 — Commit `graphify-out/`; code-only pass for now, D-007 — Written session log + decisions log for continuity (+22 more)
...
Cohesion: 0.08
Nodes (25): M1 — Media foundation (closed, pushed), M2 — Voice analysis (closed, pushed @ `e864f18`), M3 — Semantic image matching (closed, pushed @ `79b2fb2`; timing set closed @ `ca1dd23`), M4 open, M4 — Transition/animation engine (closed, pushed @ `0234747`), M5 — Caption engine (closed, pushed), M6 — Module 1: font-system (SPEC-font-system.md, tasks/plan-font.md), M6 — Module 2: preset-registry (SPEC-preset-registry.md, tasks/plan-presets.md) (+17 more)
...
Cohesion: 0.13
Nodes (4): ClipBlock(), TransitionsPanel(), EditorActions, useEditorStore
...
Cohesion: 0.22
Nodes (8): Consistent error semantics for the sidecar API., EmbedFailure, match(), _parse_beats(), Exception, Request, UploadFile, Embedding/model step failed; distinguishes embed errors from assignment errors.
...
Cohesion: 0.13
Nodes (25): isAsset(), isClip(), isRecord(), isTrack(), isTranscript(), parseCaptions(), parseProjectJson(), parseProjectModel() (+17 more)
...
Cohesion: 0.13
Nodes (41): get_config(), ApiError, error_response(), Exception, is_allowed_font_name(), _all_presets(), api_error_handler(), _builtin_by_id() (+33 more)
...
Cohesion: 0.07
Nodes (51): _candidate_names(), clean_family_name(), _decode_name(), extract_family_name(), font_license_from_payload(), FontError, has_font_signature(), load_registry() (+43 more)
...
Cohesion: 0.13
Nodes (17): attachPauses(), buildNodes(), confidenceLabel(), lowConfidence(), Node, TranscriptPanel(), Transcript, TranscriptPause (+9 more)
...
Cohesion: 0.10
Nodes (29): MatchPanel(), Beat, labelFor(), PAUSE_BEAT_THRESHOLD, segmentBeats(), splitSegment(), wordsInPart(), hasTimingOverride() (+21 more)

### Community 127 - "detect_cuts"
Cohesion: 0.13
Nodes (17): _bg_estimate(), detect_cuts(), load_analysis_image(), ndarray, Convert to gray float analysis image; returns (gray, ana_w, ana_h, factor)., Background color = mode of the 1px outer ring (comic margins). Panels carry…, row_features(), clean_fixture() (+9 more)
...
Cohesion: 0.08
Nodes (28): MOTION_TYPES, MotionPanel(), applyClips(), clipWith(), imageAsset, unmount(), videoAsset, imageA (+20 more)
...
### Community 145 - "InspectorPanel.tsx"
Cohesion: 0.19
...
### Community 155 - "detect.py"
Cohesion: 0.29
Nodes (9): _bordered_by_content(), find_gutter_bands(), GutterBand, M7 module 2: hybrid panel detection (OpenCV + numpy). Operates on a downscaled…, Both immediate sides of the [start, end) run must be strong content., 1..SEAM_MAX_H flat empty runs between clean bands, bordered by content., _rescue_cuts(), RowFeatures (+1 more)
...
## Knowledge Gaps
- **1156 isolated node(s):** `$schema`, `plugin`, `lava-backend`, `$schema`, `plugins` (+1151 more)
...
- **Why does `vitest` connect `vitest` to `package.json`, `transitions.ts`, `fontStore.ts`, `TrackRow.tsx`, `ops.ts`, `editorStore.ts`, `ffmpeg.ts`, `App.tsx`, `editor/presets.ts`, `InspectorPanel.tsx`, `captions.ts`, `project.ts`, `TranscriptPanel.tsx`, `PresetPanel.test.tsx`, `TemplateEditorPanel.test.tsx`, `matchingStore.ts`?**
  _High betweenness centrality (0.014) - this node is a cross-community bridge._
- **Why does `useEditorStore` connect `useEditorStore` to `transitions.ts`, `TrackRow.tsx`, `ops.ts`, `editorStore.ts`, `App.tsx`, `editor/presets.ts`, `InspectorPanel.tsx`, `captions.ts`, `project.ts`, `TranscriptPanel.tsx`, `PresetPanel.test.tsx`, `TemplateEditorPanel.test.tsx`, `matchingStore.ts`?**
  _High betweenness centrality (0.008) - this node is a cross-community bridge._
- **Why does `parse_captions()` connect `parse_captions` to `main.py`?**
